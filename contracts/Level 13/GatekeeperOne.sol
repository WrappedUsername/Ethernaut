// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

// Not needed for this pragma version.
import '@openzeppelin/contracts/utils/math/SafeMath.sol';

contract GatekeeperOne {

  /** @notice Using for directive, using imported SafeMath functions from openzeppelin
  for unit256 to prevent overflow and underflow attacks. Not needed though, author used
  pragma version ^0.6.0, I decided to leave it to preserve the original contract as much 
  as possible. */
  using SafeMath for uint256;
  address public entrant;
  uint256 i = uint256(uint160(address(tx.origin)));

  modifier gateOne() {
    require(msg.sender != tx.origin);
    _;
  }

  modifier gateTwo() {
    require(gasleft().mod(8191) == 0);
    _;
  }

  modifier gateThree(bytes8 _gateKey) {
      require(uint32(uint64(_gateKey)) == uint16(uint64(_gateKey)), "GatekeeperOne: invalid gateThree part one");
      require(uint32(uint64(_gateKey)) != uint64(_gateKey), "GatekeeperOne: invalid gateThree part two");
      require(uint32(uint64(_gateKey)) == uint16(i), "GatekeeperOne: invalid gateThree part three");
    _;
  }

  function enter(bytes8 _gateKey) public gateOne gateTwo gateThree(_gateKey) returns (bool) {
    entrant = tx.origin;
    return true;
  }
}